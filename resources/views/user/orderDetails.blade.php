...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-16 py-10">
        <h2 class="text-3xl font-bold text-gray-800 mb-8">Order {{ $order->invoice_no }}</h2>


        <div class="bg-gradient-to-r from-blue-50 to-white border border-blue-100 rounded-lg p-4 mb-6 shadow-sm">
            <p class="text-sm text-gray-600 mb-1">Order placed on
        ...
        <div class="bg-white border rounded-xl shadow-sm overflow-hidden mb-10">
            <div class="px-6 py-4 border-b">
                <h3 class="text-xl font-semibold text-gray-800">Products in this Order</h3>
            </div>
                ...
                <table class="w-full text-sm text-gray-700">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="p-4 text-left">Color / Size</th>
                    ...
                    <tbody>
                        @foreach ($order->items as $item)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="p-4">{{ $item->color ?? '-' }} / {{ $item->size ?? '-' }}</td>
        ...
        <div class="text-right mt-6">
            <a href="{{ route('user.invoice.download', $order->id) }}"
                class="inline-flex items-center gap-2 bg-[#0B1D51] hover:bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium shadow">
