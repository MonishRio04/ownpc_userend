...
@section('content')
    ...
    <div class="max-w-5xl mx-auto px-4 py-10">
        ...
        <div
            class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6 px-10 bg-white dark:bg-black dark:text-white shadow rounded p-4">
            <a href="#cod"
                class="tab-button active-tab block text-center rounded-lg shadow px-4 py-6 font-semibold cursor-pointer transition duration-200"
                onclick="showTab(event, 'cod')">Cash on Delivery</a>
            <a href="#card"
                class="tab-button block text-center rounded-lg shadow px-4 py-6 font-semibold cursor-pointer transition duration-200"
                onclick="showTab(event, 'card')">Credit/Debit Card</a>
            <a href="#netbanking"
                class="tab-button block text-center rounded-lg shadow px-4 py-6 font-semibold cursor-pointer transition duration-200"
                onclick="showTab(event, 'netbanking')">Net Banking</a>
            <a href="#paypal"
                class="tab-button block text-center rounded-lg shadow px-4 py-6 font-semibold cursor-pointer transition duration-200"
                onclick="showTab(event, 'paypal')">PayPal Account</a>
        </div>
        ...
        <div id="netbanking" class="tab-content hidden bg-white shadow rounded p-6 dark:bg-black dark:text-white">
            ...
            <div class="space-y-2">
                ...
                <select name="other_bank"
                    class="w-full mt-1  border-gray-300 rounded px-4 py-2 bg-gray-100 focus:outline-none focus:ring-2 focus:ring-none">
        ...
        <div id="paypal" class="tab-content hidden bg-white shadow rounded p-6 dark:bg-black dark:text-white">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                ...
                <div class="space-y-5">
                    ...
                    @endforeach
                    <button class="w-full mt-4 hover:bg-[#0B1D51] bg-orange-400 text-white px-6 py-3 rounded">Proceed
                        Payment</button>
                </div>
            </div>
        </div>
