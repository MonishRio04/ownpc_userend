...
<div class="max-w-7xl mx-auto p-16 space-y-12 text-black dark:text-white">
    ...
    <div id="cartWrapper">
            ...
            <tbody>
                @foreach ($cart as $item)
                    <tr class="border-t dark:border-gray-700">
                        ...
                        <td class="p-3">
                            <img src="{{ asset($item->options->image) }}" class="w-16 h-16" />
                        </td>
                        ...
                        <td class="p-3">
                            <button type="button" class="remove-cart-item" data-rowid="{{ $item->rowId }}"
                                title="Remove Item">
                                <i class="fa-regular fa-circle-xmark text-2xl text-red-500 hover:text-red-700"></i>
                            </button>
        ...
        <div class="text-right space-y-2 mt-6">
            ...
            <p class="text-xl font-bold text-orange-500 cart-total">
                Total: ₹{{ Cart::total() }}
            </p>
        </div>
    </div>
    <form action="{{ route('Payment') }}" method="get" onsubmit="return validateCheckoutForm();">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 shadow rounded-xl space-y-6">
            <h2 class="text-2xl font-bold">Add Your Details</h2>

            <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                <input type="text" name="name" placeholder="Full Name" required
                    class="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 dark:text-white rounded focus:outline-none">
                ...
                <select name="address_type" required
                    class="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 dark:text-white rounded">
            ...
            <div>
                <button type="submit"
                    class="inline-flex items-center gap-2 hover:bg-[#0B1D51] text-white px-6 py-2 rounded-md text-lg font-semibold bg-orange-400 dark:hover:bg-orange-500">
                    MAKE A PAYMENT
